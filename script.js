...
async function fetchProductData() {
    try {
        ...
        const data = await response.json(); // Read all data into a temporary variable

        // 1. Generate Hotspots first
        if (data.hotspots) {
            generateHotspots(data.hotspots);
        }

        // 2. Separate hotspot data from product data
        const { hotspots, ...productData } = data;
        allProductsData = productData; // Store only the product data for content generation

        console.log("Product data and hotspot data loaded successfully.");

        // Generate the HTML from the remaining product data
        generateInitialHTML(allProductsData);

        // Now that the HTML is in the DOM, initialize carousels
        initCarousels();
    } catch (error) {
        console.error("Could not fetch product data. Check if products.json exists and if you are using a local web server:", error);
    }
}
